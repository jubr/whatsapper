...
GitHub ref lists are cached for 1 week; a failed fetch still serves that cache with `Seeing results cached from YYYY-MM-DD hh:mm:ss`.
Choose `[Other github repo]` (above built-in) to paste a full GitHub path + branch (for example `https://github.com/wwebjs/whatsapp-web.js/tree/stealth`). That URI is persisted until `[Built-in]` is selected.
